...
#### 【强制】禁用非标准缩写（包括非标准首字母缩略词）。 相对节约横向空间来说，对于一个新的读者来说，能够立即理解意义会更为重要，例如：
```objc
// GOOD:
...
#### 【强制】任何类、扩展、方法、函数或者变量名中简称或者缩略语必须大写。对于名字，可参照苹果缩略语或简称大写字母标准，例如URL, ID, TIFF, EXIF。
...
#### 【推荐】方法声明，如最开始的示例所示，声明一个 @interface 内容的推荐顺序为：属性，类方法，初始化方法，实例方法。类方法区域，把便捷初始化方法放在前面。
